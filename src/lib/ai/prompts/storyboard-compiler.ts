...
export function compileStoryboardMarkdown(segment: StoryboardSegment): string {
  ...
  for (const p of segment.panels) {
    lines.push(
      ...
      '',
      `**VOICEOVER / DIALOG**`,
      ...(p.voiceoverScript ? [`Speaker: ${p.voiceoverSpeaker ?? 'narrator'}`, `Script: ${p.voiceoverScript}`] : ['Script: (tidak ada)']),
      ...(p.voiceDirection ? [`Direction: ${p.voiceDirection}`] : []),
      ...(p.soundDesign ? [`Sound Design: ${p.soundDesign}`] : []),
      ...(p.dialogueVo ? ['', `**Dialog/VO (alt):** ${p.dialogueVo}`] : []),
      '',
      `**DIALOG/VO**`,
